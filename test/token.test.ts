...
test('token metadata with OpenZeppelin decimals field', () => {
  // OpenZeppelin fungible token uses 'decimals' (plural) in its Metadata struct
  // decimals: 18, name: "Soroban Token", symbol: "SOR", wasm executable (non-SAC)
  const xdr_string =
    '';
  const resouces = TokenMetadata.fromLedgerEntryData(xdr_string);
  expect(resouces.name).toEqual('Soroban Token');
  expect(resouces.symbol).toEqual('SOR');
  expect(resouces.decimals).toEqual(18);
